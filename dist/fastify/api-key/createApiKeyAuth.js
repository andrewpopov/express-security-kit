...
async function unauthorized(reply) {
    await reply.code(401).send({
        error: { code: 'UNAUTHORIZED', message: 'Unauthorized' },
    });
}
/** Send a generic 403. */
async function forbidden(reply) {
...
function createApiKeyAuth(config) {
    ...
    const fail = async (request, reply, reason, status) => {
        ...
        if (config.onFailure) {
            try {
                const maybePromise = config.onFailure(request, reason);
                if (maybePromise &&
                    typeof maybePromise.then === 'function') {
                    maybePromise.catch((err) => logger.warn('[express-security-kit/fastify] onFailure hook rejected', err));
                }
            }
            catch (err) {
    ...
    return async (request, reply) => {
        const outcome = await (0, verifyApiKey_1.verifyApiKey)(config, request);
        if (outcome.ok) {
