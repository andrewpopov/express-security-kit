export interface RedactUrlOptions {
   ...
   */
  sensitiveSegments?: { afterSegments: string[] };
 ...
 * exact-match chance).
 */
function decodeForCompare(token: string): string {
  try {
...
function redactQuery(query: string, sensitiveLower: Set<string>, placeholder: string): string {
  if (sensitiveLower.size === 0 || query.length === 0) return query;
  return query
    .split('&')
    .map((pair) => {
      const [key] = splitOnce(pair, '=');
      if (!sensitiveLower.has(decodeForCompare(key).toLowerCase())) return pair;
...
function redactPath(path: string, markersLower: Set<string>, placeholder: string): string {
  if (markersLower.size === 0) return path;
 ...
 */
function redactUserInfo(url: string, placeholder: string): string {
...
function doRedact(url: string, options: RedactUrlOptions): string {
  const placeholder = options.placeholder ?? 'REDACTED';
  const sensitiveLower = new Set((options.sensitiveParams ?? []).map((p) => p.toLowerCase()));
  const markersLower = new Set(
    (options.sensitiveSegments?.afterSegments ?? []).map((s) => s.toLowerCase()),
  );
